          ...
          m_shutter_mode(0),
{
    ...
    if (GetAvailableCameras(&numCameras)!= _DRV_SUCCESS)
    {
    ...
    else
    {
        ...
        throw LIMA_HW_EXC(Error, "Invalid Camera number ")
    }
    ...
    {
        DEB_ERROR() << "Library initialization failed, check the config. path" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Library initialization failed, check the config. path");       
    }
    ...
    {
        DEB_ERROR() << "Cannot get camera capabilities" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get camera capabilities");            
    }
    ...
    {
        DEB_ERROR() << "Cannot get camera model: " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get camera model");            
    }
    m_detector_model = model;
    m_detector_type = m_camera_capabilities.ulCameraType;
    
    DEB_TRACE() << "Andor Camera device found:" << endl
                << "    * Type  : " << AndorCcdTypes[m_detector_type] << endl
    ...
    {
        DEB_ERROR() << "Cannot set camera read mode" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot camera read mode");                
    }
    int xbin_max, ybin_max;
    
    if (andorError(GetMaximumBinning(m_read_mode, 0, &xbin_max)))
    {
        DEB_ERROR() << "Cannot get the horizontal maximum binning" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get the horizontal maximum binning");            
    }
    if (andorError(GetMaximumBinning(m_read_mode, 1, &ybin_max)))
    {
        DEB_ERROR() << "Cannot get the vertical maximum binning" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get the vertical maximum binning");            
    }
    ...
    m_trig_mode_maps[ExTrigSingle] = 1;
    m_trig_mode_maps[ExGate] = 7;
...
Camera::~Camera()
{
    ...
    if (m_cooler)
    {
        string error_str << "Please stop the cooling before shuting dowm the camera" <<endl
                        << "brutale heating could damage the sensor." << endl
                        << "And wait until temperature rises above 5 deg, before shuting down."
        DEB_ERROR() << error_str;
        throw LIMA_HW_EXC(Error, error_str);
    }
...
void Camera::startAcq()
{
    ...
    {
        DEB_ERROR() << "Cannot get status" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get status");            
    }
    if (status != _DRV_IDLE)
    {
        _setStatus(Camera::Fault,false);        
        DEB_ERROR() << "Cannot start acquisition, camera is not idle" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot start acquisition, camera is not idle");            
    }   
    ...
    {
        DEB_ERROR() << "Cannot get size of circular buffer" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get size of circular buffer");            
    }        
    ...
    if (andorErr(StartAcquisition()))
    {
        DEB_ERROR() << "Cannot start acquisition" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot start acquisition");            
    }
}
...
void Camera::_stopAcq(bool internalFlag)
{
  ...
  if(m_status != Camera::Ready)
  {
      ...
      {
          DEB_ERROR() << "Cannot abort acquisition" << " : error code = " << m_camera_error;
          throw LIMA_HW_EXC(Error, "Cannot abort acquisition");
      }
...
void Camera::_AcqThread::threadFunction()
{
    ...
    while(!m_cam.m_quit)
    {
        ...
        while(continueAcq && (!m_cam.m_nb_frames || m_cam.m_image_number < m_cam.m_nb_frames))
        {
            ...
            long first, last;
            if (andorError(GetNumberNewImages(&first, &last)))
            {
                if (m_camera_error == _DRV_NO_NEW_DATA) continue;
                else
                {
                    DEB_ERROR() << "Cannot get number of new images" << " : error code = " << m_camera_error;
                    throw LIMA_HW_EXC(Error, "Cannot get number of new images");
                }
            }        
            ...
            for (long im=first; im <= last; im++)
            {
                ...
                if (andorError(GetImages(im, im,(at_32 *) ptr, (unsigned long)size,&validfirst, &validlast)))
                {
                    m_cam._setStatus(Camera::Fault,false);
                    continueAcq = false;
                    DEB_ERROR() << "Cannot get image #" << im << " : error code = " << m_camera_error;
                    throw LIMA_HW_EXC(Error, "Cannot get last image");                
                }
...
void Camera::getDetectorImageSize(Size& size)
{
    ...
    {
        DEB_ERROR() << "Cannot get detector size" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get detector size");                    
    }     
    size= Size(xmax, ymax);
}
...
void Camera::getImageType(ImageType& type)
{
    ...
    {
        DEB_ERROR() << "Cannot get detector bit depth" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get detector bit depth");                        
    }
...
void Camera::getDetectorType(string& type)
{
    ...
    type = AndorCcdTypes[m_detector_type];
}
        ...
        case ExtGate:
            ...
            switch (m_error_code)
            {
                case _DRV_SUCCESS:
                    valid_mode = true;
                    break;
                case _DRV_INVALID_MODE;
                    valid_mode = false;
                    break;
                case _DRV_NOT_INITIALIZED:                
                    valid_mode = false;
                    DEB_ERROR() << "System not initializsed, cannot get trigger mode status" << " : error code = " << m_camera_error;
                    throw LIMA_HW_EXC(Error, "System not initializsed, cannot get trigger mode status");
                    break;                                                     
            }                
        break;
...
void Camera::setExpTime(double exp_time)
{
    ...
    {
        DEB_ERROR() << "Cannot set exposure time" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot set exposure time");                        
    }
...
void Camera::getExpTime(double& exp_time)
{
    ...
    {
        DEB_ERROR() << "Cannot get acquisition timings" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get acquisition timings");                        
    }
    m_exp_time = exp;
    m_kin_time = kin;
...
void Camera::setLatTime(double lat_time)
{
    ...
    {
        DEB_ERROR() << "Cannot set kinetic cycle time" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot set kinetic cycle time");                        
    }
...
void Camera::getExposureTimeRange(double& min_expo, double& max_expo) const
{
    ...
    float max;
    min_expo = 0.;
    if andorError(GetMaximumExposure(&max)))
    {
        DEB_ERROR() << "Cannot get the maximum exposure time: " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get the maximum exposure time");    
    }    
    max_expo = (double) max;
...
void Camera::setNbFrames(int nb_frames)
{
    ...
    if (nb_frames == 0)
    {
        ...
        throw LIMA_HW_EXC(Error, "Sorry continuous acquisition (setNbFrames(0)) not yet implemented");                
    }
    // --- We only work on kinetics mode which allow multi-frames to be taken
    // ---
    if (andorError(SetNumberKinetics(nb_frames)))
    {
        DEB_ERROR() << "Cannot set number of frames << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot set number of frames");            
    }        
...
void Camera::setRoi(const Roi& set_roi)
{
    ...
    if(set_roi.isActive())
    {
        ...
        if (andorError(SetImage(b.getX(), b.getY(), hstart, hend, vstart, vend)))
        {
            DEB_ERROR() << "Cannot set detector ROI" << " : error code = " << m_camera_error;
            throw LIMA_HW_EXC(Error, "Cannot set detector ROI");                                        
        }
        m_roi = set_roi;
    }
}
...
void Camera::setBin(const Bin &set_bin)
{
    ...
    if(set_roi.isActive())
    {
        ...
        if (andorError(SetImage(b.getX(), b.getY(), hstart, hend, vstart, vend)))
        {
            DEB_ERROR() << "Cannot set detector BIN" << " : error code = " << m_camera_error;
            throw LIMA_HW_EXC(Error, "Cannot set detector BIN");                                        
        }
        m_bin = set_bin;
    }
...
bool Camera::isBinnigAvailable(void)
{
    ...
    // --- ok not realy need this function but could we completed
...
//- Camera::getPixelSize()
//-----------------------------------------------------
bool Camera::getPixelSize(double& size)
{
    DEB_MEMBER_FUNCT();
    int xsize, ysize;
    ...
    {
        DEB_ERROR() << "Cannot pixel sizes" << " : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get pixel size");                                        
    }
...
//- Camera::andorError()
//-----------------------------------------------------
bool Camera::andorError(AndorErrorCode code)
{
    m_camera_error = 0;
...
//-----------------------------------------------------
void Camera::InitialiseController(void)
{
        // --- Init adc / speed
        initAdcSpeed();
...
void Camera::initAdcSpeed(void)
{
    ...
    {
        DEB_ERROR() << "Cannot get number of ADC" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get number of ADC");            
	}
	...
	for (ia=0; ia<nadc; ia++) {
		if (andorError(GetNumberHSSpeeds(ia, 0, &nSpeed[ia]))) {
			DEB_ERROR() << "Cannot get nb of Horizontal Speed for ADC " <<  ia <<" : error code = " << m_camera_error;
			throw LIMA_HW_EXC(Error, "Cannot get nb of Horizontal Speed for an ADC");            
		}
	...
	for (ia=0; ia<nadc; ia++) {
		for (ih=0; ih<nSpeed[ia]; ih++) {
			if (andorError(GetHSSpeed(ia, 0, ih, &m_adc_speeds[is].speed))) {
                DEB_ERROR() << "Cannot get Horizontal Speed " << ih << " for ADC " << ia <<" : error code = " << m_camera_error;
                throw LIMA_HW_EXC(Error, "Cannot get Horizontal Speed ");            
			}
...
void Camera::setAdcSpeed(int adc)
{
    ...
    // -- Initialise ad speed
    if ((adc == -1) || (adc > m_adm_speed_number))
    {
        is = m_adc_speed_max;
    }
    else
    ...
    if (andorError(SetADCChannel(m_adc_speeds[is].adc)))
    {
        DEB_ERROR() << "Failed to set ADC channel #" << m_adc_speeds[is].adc <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set ADC channel");            
    }
    if (andorError(SetHSSpeed(0, m_adc_speeds[is].hss)))
    {
        DEB_ERROR() << "Failed to set HSS #" << m_adc_speeds[is].hss <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set HSS");            
    }    
    m_adc = is;
...
void Camera::initVSS(void)
{
    ...
    {
        DEB_ERROR() << "Cannot get number of possible VSS" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Cannot get number of possible VSS");            
	}
	...
	for (ivss=0; m_vss_number; ivss++)
    {
        ...
        {
			DEB_ERROR() << "Cannot get VSS value for #" << ivss <<" : error code = " << m_camera_error;
            throw LIMA_HW_EXC(Error, "Cannot get VSS value");
			return (DS_NOTOK);
		}
	}
    ...
    {
		DEB_ERROR() << "Cannot get recommended VSS speed. Set it to 0" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error,"Cannot get recommended VSS speed. Set it to 0"
		m_vss_best = 0;
	}
}
...
void Camera::setVSS(int vss) 
{
    ...
    {
		DEB_ERROR() << "Failed to set VSS #" << is <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error,"Failed to set VSS");
	}
	m_vss = is;
...
void Camera::initPGain(void)
{
    ...
    {
		DEB_ERROR() << "Failed to get number of preamp gain" <<" : error code = " << m_camera_error;
		throw LIMA_HW_EXC(Error,"Failed to get number of preamp gain");
	}
	...
	for (ig=0; ig<m_gain_number; ig++)
    {
        ...
        {
			DEB_ERROR() << "Failed to get gain #" << ig <<" : error code = " << m_camera_error;
			throw LIMA_HW_EXC(Error,"Failed to get gain");
		}
...
void Camera::setPGain(int gain) 
{
    ...
    {
		DEB_ERROR() << "Failed to set Preamp Gain #" << ig <<" : error code = " << m_camera_error;
		throw LIMA_HW_EXC(Error,"Failed to set Preamp Gain");
	}
	m_gain= ig;
...
void Camera::setFastExtTrigger(bool flag)
{
    ...
    {
        DEB_ERROR() << "Failed to set ext-trigger fast mode" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set ext-trigger fast mode");      
    }
    m_fasttrigger = flag;
...
void Camera::setShutterLevel(int level);
{
    ...
    {
        DEB_ERROR() << "Failed to set shutter level" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set shutter level");          
    }
...
void Camera::setShutterOpenTime(int tm);
{
    ...
    {
        DEB_ERROR() << "Failed to set shutter openning time" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set shutter opening time");          
    }
...
void Camera::setShutterCloseTime(int tm);
{
    ...
    {
        DEB_ERROR() << "Failed to set shutter closing time" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set shutter closing time");          
    }
...
void Camera::setTemperatureSP(int temp);
{
    ...
    {
        DEB_ERROR() << "Failed to set temperature set-point" <<" : error code = " << m_camera_error;
        throw LIMA_HW_EXC(Error, "Failed to set temperature set-point");          
    }
...
void Camera::getTemperature(int& temp);
{
    ...
    int tm;
    unsigned int status;
    if ((status = GetTemperature(&tm)) == _DRV_ERROR_ACK)
    {
        DEB_ERROR() << "Failed to read temperature" <<" : error code = " << _DRV_ERROR_ACK;
        throw LIMA_HW_EXC(Error, "Failed to read temperature");          
    }
    temp = tm;   
}
//-----------------------------------------------------
...
void Camera::setCooler(bool flag);
{
    ...
    if (flag)
    {
        ...
        {
            DEB_ERROR() << "Failed start the cooler" <<" : error code = " << m_camera_error;
            throw LIMA_HW_EXC(Error, "Failed to start the cooler");          
        }
    ...
    else
    {
        ...
        {
            DEB_ERROR() << "Failed to stop the cooler" <<" : error code = " << m_camera_error;
            throw LIMA_HW_EXC(Error, "Failed to stop the cooler");          
        }
    }
    m_cooler = flag;        
}
...
//
//-----------------------------------------------------
void Camera::getCoolerStatus(string& status);    
{
    DEB_MEMBER_FUNCT();
